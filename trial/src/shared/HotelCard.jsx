...
const HotelCard = ({tour}) => {
    ...
    const {id, reviews,title,city, photo,price,featured}=tour;
  ...
  <h5 className='tour__title'><Link to={`/tours/${id}`}>{title}</Link></h5>
   <div className='card__bottom d-flex align-items-senter justify-content-between mt-3'>
       <h5>${price} <span>/per person </span></h5>

    <button className='btn booking__btn'>
        <Link  className="btn1" to={`/tours/${id}`}>Book Now</Link>
    </button>
